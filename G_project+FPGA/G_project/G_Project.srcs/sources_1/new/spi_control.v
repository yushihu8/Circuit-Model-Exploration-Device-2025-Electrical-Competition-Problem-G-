/**********************************************************
>> 模块名       :SPI控制模块
>> 信息         :控制SPI与内部寄存器的交互
>> 版本         :v0.0
>> 时间         :2025-11-21
>> 作者         :ts
************************************************************/

module spi_control(
    clk,
    rstn,

    spi_cs,
    spi_clk,
    spi_mosi,
    spi_miso,

    w_reg_addr,
    w_reg_data,
    w_reg_en,

    r_reg_addr,
    r_reg_data,
    r_reg_en
  );

  input clk;
  input rstn;

  input spi_cs;
  input spi_clk;
  input spi_mosi;
  output spi_miso;

  output reg w_reg_en;
  output reg[15:0] w_reg_addr;
  output [15:0] w_reg_data;

  output reg r_reg_en;
  output reg[15:0] r_reg_addr;
  input  [15:0]r_reg_data;

  reg [7:0] state;
  reg [7:0] state_next;
  reg [7:0] state_return;

  reg [7:0]mode_reg;

  wire [15:0] spi_data_out;
  wire        spi_rec_val;
  reg         spi_data_val;


  assign w_reg_data = spi_data_out;

  localparam IDLE = 8'h00,
             WRITE_ADDR = 8'h02,
             WRIET_REG= 8'h03,
             WRITE_REG_AADR_CHANGE = 8'h04,
             READ_REG_EN = 8'h05,
             READ_REG = 8'h06,
             READ_REG_AADR_CHANGE = 8'h07;

  localparam HEAD_FRAME = 8'haa,
             LAST_FRAME = 16'ha5a5,
             MODE_REG_WRITE_SINGLE= 8'h01,
             MODE_REG_WRITE_CURRETN = 8'h02,
             MODE_REG_READ_SINGLE = 8'h03,
             MODE_REG_READ_CURRETN = 8'h04;

  spi_slave spi_slave_inst0 (
              .clk(clk),
              .rstn(rstn),
              .spi_cs(spi_cs),
              .spi_clk(spi_clk),
              .spi_mosi(spi_mosi),
              .spi_miso(spi_miso),
              .spi_data_in(r_reg_data),
              .spi_data_out(spi_data_out),
              .spi_rec_val(spi_rec_val),
              .spi_data_val(spi_data_val)
            );

  always @(posedge clk or negedge rstn)
  begin
    if(~rstn)
      state <= IDLE;
    else
      state <= state_next;
  end

  always @(*)
  begin
    case (state)
      IDLE:
      begin
        if(spi_rec_val && spi_data_out[15:8] == HEAD_FRAME)
          state_next = WRITE_ADDR;
        else
          state_next = IDLE;
      end
      WRITE_ADDR:
      begin
        if(spi_rec_val)
        begin
          case (mode_reg)
            MODE_REG_WRITE_SINGLE:
              state_next = WRIET_REG;
            MODE_REG_WRITE_CURRETN:
              state_next = WRIET_REG;
            MODE_REG_READ_SINGLE:
              state_next = READ_REG_EN;
            MODE_REG_READ_CURRETN:
              state_next = READ_REG_EN;
            default:
              state_next = IDLE;
          endcase
        end
        else
          state_next = WRITE_ADDR;
      end
      WRIET_REG:
      begin
        if(mode_reg == MODE_REG_WRITE_SINGLE)
        begin
          if(spi_rec_val)
            state_next = IDLE;
          else
            state_next = WRIET_REG;
        end
        else
        begin
          if(spi_rec_val)
          begin
            if(spi_data_out == LAST_FRAME)
              state_next = IDLE;
            else
              state_next = WRITE_REG_AADR_CHANGE;
          end
          else
            state_next = WRIET_REG;
        end
      end
      WRITE_REG_AADR_CHANGE:
        state_next = WRIET_REG;
      READ_REG_EN:
        state_next = READ_REG;
      READ_REG:
      begin
        if(mode_reg == MODE_REG_READ_SINGLE)
        begin
          if(spi_rec_val)
            state_next = IDLE;
          else
            state_next = READ_REG;
        end
        else
        begin
          if(spi_rec_val)
          begin
            if(spi_data_out == LAST_FRAME)
              state_next = IDLE;
            else
              state_next = READ_REG_AADR_CHANGE;
          end
          else
            state_next = READ_REG;
        end
      end
      READ_REG_AADR_CHANGE:
        state_next = READ_REG_EN;
      default:
        state_next = IDLE;
    endcase
  end
  reg r_reg_en_temp;
  always @(posedge clk or negedge rstn)
  begin
    if(~rstn)
    begin
      mode_reg <= 8'd0;

      spi_data_val <= 1'd0;

      w_reg_en <= 1'd0;
      w_reg_addr <= 16'd0;

      r_reg_en <= 1'd0;
      r_reg_en_temp<= 1'd0;
      r_reg_addr <= 16'd0;
    end
    else
    begin
      case (state)
        IDLE:
        begin
          spi_data_val <= 1'd0;

          w_reg_en <= 1'd0;

          r_reg_en <= 1'd0;
          r_reg_en_temp <= 1'd0;
          if(spi_rec_val && spi_data_out[15:8] == HEAD_FRAME)
            mode_reg <= spi_data_out[7:0];
        end
        WRITE_ADDR:
        begin
          if(spi_rec_val)
          begin
            r_reg_addr <= spi_data_out;
            w_reg_addr <= spi_data_out;
          end
        end
        WRIET_REG:
        begin
          if(spi_rec_val && spi_data_out != LAST_FRAME)
            w_reg_en <= 1'd1;
          else
            w_reg_en <= 1'd0;
        end
        WRITE_REG_AADR_CHANGE:
        begin
          w_reg_en <= 1'd0;
          w_reg_addr <= w_reg_addr + 16'd1;
        end
        READ_REG_EN:
          r_reg_en <= 1'd1;
        READ_REG:
        begin
          r_reg_en <= 1'd0;
          r_reg_en_temp <= r_reg_en;
          spi_data_val <= r_reg_en_temp;
        end
        READ_REG_AADR_CHANGE:
          r_reg_addr <= r_reg_addr + 16'd1;
        default:
        begin

        end
      endcase
    end
  end

endmodule

